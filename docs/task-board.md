...
Phase 4 is complete. See [the Phase 4 record](phase-4-first-complete-use.md) for the verified browser, worker, Action Simulator, database, and receipt behavior. The next implementation phase is **Phase 5 — lost acknowledgement and exact retry**.

1. Add lost-ack fault control and exact retry recovery while preserving one use and one effect.
2. Keep the pending operation durable across acknowledgement loss and browser refresh.
3. Prove retry responses return the original receipt with zero additional usage or action delta.

The three-use bound, evidence, and the guided judge flow remain later phases.

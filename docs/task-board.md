...
Phase 2 is complete. See [the protocol record](phase-2-protocol.md) for verified behavior and the simulated provider's limits. The next implementation phase is **Phase 3 — database foundation and durable acceptance**.

1. Recheck current Git/runtime state and read the Phase 3 transaction and privacy requirements.
2. Add verifier and Action Simulator product migrations, constraints, indexes, and separate role grants.
3. Implement policy/challenge repositories and the atomic presentation acceptance transaction against the frozen contracts and crypto verifier.
4. Prove commit/rollback, uniqueness, retry lookup ordering, and raw-proof/nullifier non-persistence with real PostgreSQL tests before exposing acceptance as complete.

The browser wallet, complete external action flow, safe lost-ack recovery, three-use bound, and live evidence remain later phases.
